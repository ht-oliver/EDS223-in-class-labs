...
map1 <- tm_shape(zion_elevation)+
  tm_raster(col.legend = tm_legend("Elevation (m)")) # Legend title
...
```{r}
# Our output will have 4 different layers, for 4 different bands
landsat <- rast(system.file("raster/landsat.tif", package = "spDataLarge"))

# Find out the number of layers in our raster object
nlyr(landsat)

# Find out the names of layers
names(landsat)
```
...
merged <- merge(austria, switzerland) # Must be the same resolution
...
map1 <- tm_shape(austria) +
  tm_raster(col.legend = tm_legend("Elevation(m)"),
            col.scale = tm_scale_intervals(breaks = c(0,500,1000,1500,2000,2500,3000))) +
  tm_title(text = "Austria") # This needs to be fixed
...
# Five value summary of points
# Warning says that it used a sample - meaning it didn't look at every data point
...
```{r}
landsat[1] <- cbind(c(0), c(0), c(0), c(0)) # Same number of values as matrix layers
landsat[1]
```
...
# approach 1 : find the Cell ID for point
...
```{r}
# ind the extent of current raster
...
```{r}
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend = tm_legend(title = "Elevations (m)")) +
...
map2 <- tm_shape(masked1) +
  tm_raster(col.legend = tm_legend(title = "Elevation(m)")) +
  tm_title(text = "masked1")

map3 <- tm_shape(masked2) +
  tm_raster(col.legend = tm_legend(title = "Elevation(m)")) +
  tm_title(text = "masked2")
...
```{r}
# create a reclassificiation matrix
rcl_0 <- matrix(c(1000, 1500, 1, #group 1 ranges from 1000 to 1500 m
               1500, 2000, 2, #group 2 ranges from 1500 to 2000 m
               2000, 2500, 3, #group 3 ranges from 2000 to 2500 m
               2500, 3000, 4), #group 4 ranges from 2500 to 3000 m
             ncol = 3, byrow = TRUE)
  
# use reclass matrix to reclassify elevation raster
reclassified <- classify(zion_elevation, rcl = rcl_0)
...
```{r}
#Add empty cells to the left and right of our raster.
# This add one row to the top and one to the bottom, and 200 columns on each side
...
# Extents match, so we can perform a local operation
elev_2 + elev_3
...
```{r}
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend = tm_legend(title = "Elevatin (m)")) +
  tm_title(text = "Original")
