...
```{r include=TRUE}
# Crop landsat scene to the extent of SB County shapefile
# Crop makes a box - mask eliminates stuff outside the boundary
landsat_cropped <- crop(landsat, SB_county_south)

# Mask the raster to southern portion of SB county
landsat_masked <- mask(landsat_cropped, SB_county_south)

# Remove unnecesssary objects from environment
...
SB_classification_dt <- terra::predict(landsat, SB_decision_tree, type = "class",
                                       na.rm = TRUE)
...
# Extract the spectral bands for clustering
spectral_data <- landsat_df[,c("blue", "green", "red", "NIR", "SWIR1",
                               "SWIR2")]
...
# perform k means with k = 4. nstart specifies how many initial centroid throws to do
...
values(SB_classification_kmeans) <- NA #assign all values in that raster to NA

# Take the x,y coordinate from landsat_df and convert them to cell numbers for the raster
cells <- cellFromXY(SB_classification_kmeans, landsat_df[ , c("x", "y")])

# Update cells with cluster value
...
# cluster 3 = green vegetation (NIR highest relative to RED)
# cluster 4 = water (lowest NIR with some blue )
# cluster 1 = soil (higher red and SWIR/NIR)
# cluster 2 = urban 
