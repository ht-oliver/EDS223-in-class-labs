...
```{r}
dtm_pre <- rast(here::here( "week9", "data", "Eaton", "dtm_eaton_alignedNK_1m_PREFIRE.tif"))
dsm_pre <- rast(here::here("week9", "data", "Eaton", "dsm_eaton_alignedNK_1m_PREFIRE.tif"))
  
  
dtm_post <- rast(here::here( "week9", "data", "Eaton", "dtm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
dsm_post <- rast(here::here( "week9", "data", "Eaton", "dsm_eaton_epsg32611_geoid12b_1m_POSTFIRE.tif"))
```

```{r}
damage <- st_read(here::here("week9", "data", "06061784-f911-4d0e-aca5-0768ac66aac5.gdb")) %>% st_transform(crs = crs(dsm_post))
```


```{r}

eaton_perimeter <- st_read(here::here( "week9", "data", "Eaton","Eaton_Perimeter_20250121", "Eaton_Perimeter_20250121.shp"))%>% st_transform(crs = crs(dsm_post))
```

```{r}

# Check if all six datasets have matching crs
...
# Calculate quantiles for determining breaks - eliminate outliers/sensor errors
quantiles <- global(diff_dsm_crop, fun = quantile,
                    probs = c(.005, .995), na.rm = TRUE)
...
# Create a layer based on our elevation change
overlay <- height_shade(
  diff_mat,
  texture = colorRampPalette(c("#8B0000", "#FF6347", "#FFFFFF", 
                  "#87CEEB", "#00008B"))(256), # Pick 256 colors on this scale
  range = c(-20, 20) # Range of elevation changes
)
...
elev_mat %>% 
  ...
  # add realistic shadows at 50% intensity to enhance the 3D effect
...
```{r}
# Add elevation change and abs(elevation change) to df
