...
# Filter to the region of Canterbury
canterbury <- nz %>% 
...
```{r}
# You gotta leave that blank space for columns. [rows, columns, op]
...
```{r}
# Sparse geometry binary predicate list, either returning 'empty' or 1
# Just running one of these intersect commands will return this funky list. We want True or False
nz_height_sgbp <- st_intersects(x = nz_height, y = canterbury)
nz_height_sgbp

# Convert it to a logical vector so we can get True or False
...
```{r}
# find heights within 1000km of Canterbury
nz_height_logical <- st_is_within_distance(nz_height, canterbury,
                                           dist = units::set_units(1000, "km"),
                                           sparse = FALSE) # Makes return as a logical vector

# Filter based on a logical vectr
c_height4 <- nz_height[nz_height_logical, ]
```

Now, we should see points appear that do not *intersect* Canterbury, but are within 1000 km.
```{r}
tm_shape(nz) +
  tm_polygons()+
tm_shape(canterbury) +
...
nz_height_left_join <- st_join(nz_height, nz) %>% 
  select(id = t50_fid, elevation, region = Name) # selecting and renaming columns
...
nz_height_left_join %>% 
  group_by(region) %>% 
  summarise(n_points = n()) %>% # Returns the counts of each group (region)
  st_drop_geometry() # Drop geometry column just to speed things up
...
cycle_hire_join <- st_join(cycle_hire, cycle_hire_osm,
                           ...
                           dist = units::set_units(20, "m")) %>% # Join if the points are within 20m of each other
                        select(id, capacity)
...
# aggregate values for single points in cylce_hire_join
...
The second approach uses the `aggregate()` function. Although it doesn't follow the `dplyr` piping convention we're used to, `aggregate()` will come in handy later, so it's nice to see how it works.
...
aggregating_zones_area_weighted <- st_interpolate_aw(x = incongruent_2,
                                                     aggregating_zones,
                                                     extensive = TRUE) 
# Extensive is TRUE if we're aggregating a population, FALSE if it's a density. (Something about being intensive) Spatially extensive properties change with the size of the area, while spatially intensive properties do not.
...
#summarize the total population within each region
regions_1 <- us_states %>% 
  group_by(REGION) %>% 
...
map3 <- tm_shape(regions2) +
  tm_polygons(fill = "total_pop_15",
              fill.legend  = tm_legend("Total population")) +
  tm_title(text = "aggregate()") 
 # tm_layout(title = meta.margins = c(.2, 0, 0, 0))
...
```{r}
# Combine the geometries of wester states
...
```{r}
seine_buffer_5km <- st_buffer(seine, dist = units::set_units(5, "km"))
