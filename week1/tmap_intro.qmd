...
```{r}
# plotting a single spatial object (+ is the %>% for tmap)
...
```{r}
# You can plot multiple layers, but they will stack - the bottom entry will be on the top layer
...
```{r}
tm_shape(world_elevation) +  # Object to plot
  tm_raster() +    # How to plot - this one is raster
  tm_shape(world_vector) + # Object on top to plot
  tm_borders() +  # Makes the polygons transparent
  tm_shape(world_cities) +
  tm_dots() +
  tm_text("name", size = 0.8, col = "black" ) # Adds text
...
tm_shape(world_vector) +
  tm_borders() +
tm_shape(world_cities, is.main = TRUE) + # plotting cities last, but set as main shape - will determine zoom extent
  tm_dots() +
  tm_text("name")
...
```{r}
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + #Specify the bounding box to restrict extent, values are coordinates xmin, xmax, ymin, ymax
  tm_raster() +
tm_shape(world_vector) +
  tm_borders() 
```
...
```{r}
tm_shape(world_elevation, bbox = world_cities) + # Specify the bounding box with a dataset
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
...
```{r}
tm_shape(world_vector) +
  tm_polygons( fill = "tan",      #color of polygon interior
               col = "navyblue",  #color of borders
               lwd = 0.3,         #line width of borders
               lty = "dashed"     #border line styles
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(values = c("purple", "red")),
              fill.legend = tm_legend(title = "Life Expectancy (years)",
                                      position = tm_pos_out("left"))) # Legend positions outside of map, on the left
...
tm_shape(world_cities) +
  tm_symbols(
    size = "pop2020",
    size.legend = tm_legend(orientation = "landscape") # Legend orientation goes with symbol
  )
...
```{r}
# Adds different gridlines, these can be layered
tm_shape(world_vector) +
  tm_fill() +
  tm_graticules()
```
...
tm_shape(world_vector) +
  tm_polygons( #fill = "wb_income_region",
          # UPDATING LEGEND BREAKS AND LABELS
        fill = "gdp_per_cap",
              fill.scale = tm_scale(breaks = c(0 , 5000, 200000, 500000, Inf),
                                    labels = c("Low", "Medium", "High", "Very High")
                                    ),
               fill.legend = tm_legend(title = "Regional Income")) +
  tm_title(text = "Funky Map", frame = TRUE) +
...
tmap_save(map1, here::here("tmap_example.png"))
```
