...
print("Coordinate reference systems match")
  ...
  warning ("Updating coordinate reference to match")
...
```{r}
# Rasterize - second argument gives a second raster with the CRS you want to match
# If you don't have a raster to match to, you can create your own template raster
...
template_raster_low <- rast(ext(park_polygon), resolution = 0.05, crs= st_crs(park_polygon))

# create high spatial resolution template raster
template_raster_high <- rast(ext(park_polygon), resolution = 0.01,
...
# define raster template (ext, res, crs)
...
```{r}
#represent the number of cycle hire points
...
```{r}
# represent total capacity of cycle hire points
ch_raster3 <- rasterize(cycle_hire_osm_projected, template_raster, field = "capacity", 
                        fun = "sum", na.rm = TRUE)
...
map2 <- tm_shape(L7) +
  tm_rgb(col = tm_vars(c(4,3,1), multivariate = TRUE)) +
...
# apply nDWI function to landast bands 3&4
...
ndmi_rast <- lapp(L7[[c(4,5)]], fun = ndwi_fun)
...
tmap_arrange(map_ndwi, map_ndmi, ndvi_map)
