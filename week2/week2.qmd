...
```{r}
# Create a dataframe with information about london.
# Soon we'll combine it with the geometry
...
```{r}
# Combine geometry and data_frame
...
```{r}
world <- spData::world
class(world) #A tibble is just like a newer version of a dataframe from tidyverse
...
world_df <- st_drop_geometry(world) #Actually removes geom column
colnames(world_df)
...
#False because we gave them different CRSs
...
setdiff(cofee_data$sname_long, world$name_long)
